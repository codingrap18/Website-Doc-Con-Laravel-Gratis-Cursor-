...
@section('page-title', 'Transmittal Letters')
            ...
            <div class="p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $transmittals->total() }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $transmittals->where('date', '>=', now()->startOfMonth())->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $transmittals->sum(function($t) { return count($t->document_ids); }) }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $transmittals->unique('vendor_name')->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th class="table-header">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'transmittal_number', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}"
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <td class="table-cell">
                            <div class="text-gray-900 dark:text-white">
                                {{ $transmittal->date->format('M d, Y') }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                {{ $transmittal->date->diffForHumans() }}
                            </div>
                        </td>
                        <td class="table-cell">
                            <div class="text-gray-900 dark:text-white">
                                {{ $transmittal->vendor_name }}
                            </div>
                        </td>
                        <td class="table-cell">
                            <div class="text-gray-900 dark:text-white">
                                {{ Str::limit($transmittal->description, 50) }}
                            </div>
                        </td>
                        <td class="table-cell">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-100 dark:bg-primary-900 text-primary-800 dark:text-primary-200">
                                {{ $transmittal->getDocumentCount() }} documents
                            </span>
                        </td>
                        <td class="table-cell">
                        ...
                        <td class="table-cell">
                            <div class="flex items-center space-x-2">
                                ...
                                <button onclick="deleteTransmittal({{ $transmittal->id }})"
                                        class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-200"
...
<form id="deleteForm{{ $transmittal->id }}" method="POST" action="{{ route('transmittals.destroy', $transmittal) }}" style="display: none;">
    @csrf
    @method('DELETE')
</form>
@endforeach
