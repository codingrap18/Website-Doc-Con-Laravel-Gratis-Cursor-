...
@section('page-title', 'Create Transmittal Letter')
...
<div class="max-w-4xl mx-auto space-y-6" x-data="transmittalForm()">
    ...
    <div class="bg-gradient-to-r from-primary-500 to-secondary-500 text-white rounded-lg p-6 card-3d">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold">{{ $transmittalNumber }}</h2>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg card-3d">
        <form method="POST" action="{{ route('transmittals.store') }}" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           value="{{ old('transmittal_number', $transmittalNumber) }}"
                           class="form-input @error('transmittal_number') error @enderror"
                           readonly
                           required>
                ...
                <div class="border border-gray-200 dark:border-gray-600 rounded-lg max-h-96 overflow-y-auto">
                    ...
                    <div class="divide-y divide-gray-200 dark:divide-gray-600">
                        ...
                        <label class="flex items-center p-4 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer"
                               x-show="documentMatches({{ json_encode($document->document_number . ' ' . $document->document_title) }})">
                            <input type="checkbox"
                                   name="document_ids[]"
                                   value="{{ $document->id }}"
                                   class="form-checkbox"
                                   @change="updateSelectedCount()"
                                   {{ collect(old('document_ids', []))->contains($document->id) ? 'checked' : '' }}>
                            <div class="ml-3 flex-1">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="font-medium text-gray-900 dark:text-white">{{ $document->document_number }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $document->document_title }}</p>
                                    </div>
                                    <div class="text-right">
                                        <span class="status-badge status-{{ strtolower($document->status) }}">
                                            {{ $document->status }}
                                        </span>
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Rev. {{ $document->revision }}</p>
                                    </div>
                                </div>
                                @if($document->document_position)
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $document->document_position }}</p>
                                @endif
                            </div>
                        </label>
                        @endforeach
