...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $document->document_number }}</h1>
            <p class="text-gray-600 dark:text-gray-400">{{ $document->document_title }}</p>
        </div>

        <div class="mt-4 sm:mt-0 flex flex-col sm:flex-row gap-3">
            @if(auth()->user()->isReviewer() && $document->latest_reviewer_id === auth()->id() && !in_array($document->status, ['IFC', 'IFI']))
            <a href="{{ route('documents.review', $document) }}" class="btn-primary">
                ...
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <dd class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">{{ $document->document_number }}</dd>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <dd class="mt-1 text-lg text-gray-900 dark:text-white">{{ $document->document_title }}</dd>
                        </div>
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-gray-900 dark:text-white">{{ $document->document_position ?: 'Not specified' }}</dd>
                        </div>
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-gray-900 dark:text-white">
                                ...
                                @elseif($document->target_date->isFuture())
                                <span class="text-sm text-green-600 dark:text-green-400">
                                    ({{ $document->target_date->diffForHumans() }})
                                </span>
                                @endif
                            </dd>
                        </div>
                            ...
                            <dd class="mt-1">
                                <div class="flex items-center space-x-2">
                                    <div class="w-8 h-8 bg-primary-500 rounded-full flex items-center justify-center">
                                        <span class="text-white text-sm font-medium">{{ substr($document->latestReviewer->name, 0, 1) }}</span>
                                    </div>
                                    <span class="text-gray-900 dark:text-white">{{ $document->latestReviewer->name }}</span>
                                </div>
                            </dd>
                        </div>
                        @endif

                        @if($document->submit_to_reviewer_date)
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Submitted to Reviewer</dt>
                            <dd class="mt-1 text-gray-900 dark:text-white">
                                {{ $document->submit_to_reviewer_date->format('M d, Y') }}
                            </dd>
                        </div>
                        @endif
                    </dl>
                        ...
                        <div class="border-l-4 border-primary-400 pl-4">
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center space-x-3">
                                    <div class="w-8 h-8 bg-primary-500 rounded-full flex items-center justify-center">
                                        <span class="text-white text-sm font-medium">{{ substr($review->reviewer->name, 0, 1) }}</span>
                                    </div>
                                    <div>
                                        <p class="font-medium text-gray-900 dark:text-white">{{ $review->reviewer->name }}</p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $review->reviewed_at->format('M d, Y \a\t g:i A') }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Revision {{ $review->revision }}</p>
                                </div>
                            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg card-3d overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    @if(auth()->user()->isReviewer() && $document->latest_reviewer_id === auth()->id() && !in_array($document->status, ['IFC', 'IFI']))
                    <a href="{{ route('documents.review', $document) }}" class="block w-full btn-primary text-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        Review Document
                    ...
                    @if(auth()->user()->isDocumentController() || auth()->user()->isAdmin())
                    <a href="{{ route('documents.edit', $document) }}" class="block w-full btn-secondary text-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                        Edit Document
                    ...
                    <a href="{{ route('documents.index') }}" class="block w-full btn-secondary text-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-6">
                    <div class="flow-root">
                        <ul class="-mb-8">
                                    ...
                                    <div class="relative flex items-start space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $document->created_at->format('M d, Y \a\t g:i A') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="relative flex items-start space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $document->submission_date->format('M d, Y \a\t g:i A') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @foreach($document->reviews->sortBy('reviewed_at') as $review)
                            <!-- Review -->
                            <li>
                                <div class="relative {{ $loop->last ? '' : 'pb-8' }}">
                                    ...
                                    <div class="relative flex items-start space-x-3">
                                        <div class="relative">
                                            <div class="h-10 w-10 rounded-full {{ $review->comment === 'Approved' ? 'bg-green-500' : ($review->comment === 'Not Approved' ? 'bg-red-500' : 'bg-yellow-500') }} flex items-center justify-center">
                                                @if($review->comment === 'Approved')
                                                <svg class="h-5 w-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                                </svg>
                                                @elseif($review->comment === 'Not Approved')
                                                <svg class="h-5 w-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div>
                                                <p class="text-sm text-gray-900 dark:text-white font-medium">{{ $review->comment }}</p>
                                                <p class="text-sm text-gray-500 dark:text-gray-400">by {{ $review->reviewer->name }}</p>
                                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $review->reviewed_at->format('M d, Y \a\t g:i A') }}</p>
                                            </div>
                                        </div>
                                    </div>
